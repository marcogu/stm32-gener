...
function LibraryRow({ library, onChange, onRemove, onBrowse }: { library: Library; onChange: (next: Library) => void; onRemove: () => void; onBrowse: (field: "artifact" | "path") => void }) { const set = (key: keyof Library) => (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onChange({ ...library, [key]: event.target.value }); return <div className="library-row"><div className="library-row-top"><input className="library-name" value={library.name} onChange={set("name")} placeholder="library name *" required aria-required="true" /><select value={library.type} onChange={set("type")}><option value="local-static">Local static</option><option value="local-cmake">Local CMake</option><option value="git">Git / FetchContent</option></select><button className="icon-button" onClick={onRemove} title="Remove library">×</button></div><div className="library-fields">{library.type === "git" ? <><Field label="Repository" value={library.repository} onChange={set("repository")} required /><Field label="Ref" value={library.ref} onChange={set("ref")} required /><Field label="Checkout directory" value={library.checkoutDir} onChange={set("checkoutDir")} required /></> : library.type === "local-cmake" ? <PathField label="CMake root" value={library.path} onChange={set("path")} onBrowse={() => onBrowse("path")} required /> : <PathField label="Static archive" value={library.artifact} onChange={set("artifact")} onBrowse={() => onBrowse("artifact")} required />}<Field label="CMake target" value={library.target} onChange={set("target")} /><Field label="Include directories" value={library.includeDirs} onChange={set("includeDirs")} /></div></div>; }
